...
- **Jetty (9.4.58, matching ActiveMQ 5.19.6's bundled Jetty 9.4.x) and ldaptive (1.3.3) jars** are curl'd from Maven Central at image-build time into the broker's `lib/` — they are not part of the stock ActiveMQ distribution and are needed for the JAAS/LDAP web-console auth. `JETTY_VER` must track the broker's bundled Jetty (`activemq-parent` pom `jetty9-version`).
...
- [ ] Consider ldaptive 2.x (Java-11 line) — requires rewriting the currently-unused `ldaptive-activemq-ldap` JAAS realm in `login.config` to the 2.x API. Low priority: the active auth path is ActiveMQ's own `LDAPLoginModule`, not ldaptive.
