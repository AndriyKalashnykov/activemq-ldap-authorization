...
- `.github/workflows/ci.yml` — CI (`name: CI`). **The `docker` job (image build + Trivy CVE scan) runs ONLY on tag pushes (`on: push: tags: ['*']`) — i.e. at release time, not on every PR/master push.** Rationale: a freshly-disclosed CVE in a base image or bundled jar is a vuln-DB drift whose inputs span the images, not the PR diff, so a per-PR Trivy gate red-X's every unrelated PR (this happened — see git history / the jackson-databind CVE). Functional correctness *and* image-build correctness are still gated on every PR via the e2e jobs (which build the images), so a broken Dockerfile is still caught pre-merge; only CVE freshness is deferred to release. Jobs on **PR / master push**: `changes` (`dorny/paths-filter` — doc-only changes skip the heavy jobs; CLAUDE.md/Makefile/configs re-included; the job itself is skipped on tag pushes via `if: github.ref_type != 'tag'`), `static-check` (`make static-check`: hadolint + README mermaid-lint + C4 diagram drift-check — also runs on tag pushes, see below), `test` (bats), `e2e` (compose authZ contract for queues+topics + console/hawtio LDAP login + Jolokia; builds the broker image — `needs: [changes, test]`, NOT `docker`), `e2e-samba` (Samba AD DC serves LDAP), `e2e-apacheds` (Apache DS serves the AD seed), and `ci-pass` (single aggregate required check; `if: always()` so a skipped `docker` on a PR counts as success, keeping the required check green). Jobs on **tag push**: `static-check` (runs on tags too — a release must be lint-clean before it is built; gated `if: !cancelled() && (… || github.ref_type == 'tag')` so it still runs when the tag-skipped `changes` job is absent) → then `docker` (`needs: static-check`, so lint gates the release build; matrix-builds the **five** Dockerfiles `6.2.6/`, `samba/`, `openldap/`, `hawtio/`, `apacheds-ad/` + **blocking** Trivy scan, `exit-code: '1'`; samba/openldap clean, broker+hawtio upgrade their bundled jackson-databind to clear CVE-2026-54512/54513, broker waives one residual jetty-http CVE in `.trivyignore`) + `ci-pass`. Actions SHA-pinned.
...
- **Architecture diagrams (`docs/diagrams/`)**: the README **hero** is a **C4 System Context** rendered from [`docs/diagrams/c4-context.puml`](docs/diagrams/c4-context.puml) via **C4-PlantUML** (pinned `!include` v2.13.0, modern-flat theme, `LAYOUT_LEFT_RIGHT()`, `SHOW_LEGEND()`) to a committed PNG (`docs/diagrams/out/c4-context.png`, embedded at `<img width="1000">` ≤ its 1061px intrinsic). C4-model diagrams use C4-PlantUML (never Mermaid `C4*`) per the `/architecture-diagrams` skill; the **LDAP-tree** diagram remains inline **Mermaid** (`graph TD` — non-C4). `make diagrams` renders (pinned `plantuml/plantuml`, `--user`+`_JAVA_OPTIONS=-Duser.home=/tmp`); `make diagrams-check` is the drift gate wired into `static-check`. It uses `git status --porcelain --untracked-files=all` (NOT `git diff`, which is blind to a new untracked PNG). Renderer bumps are caught via a version-stamped sentinel (`docs/diagrams/out/.plantuml-<ver>.stamp`, gitignored): a `PLANTUML_VERSION` bump changes the stamp filename → forces re-render → drift gate fires. The render is byte-deterministic across runs (verified), so the gate never false-positives.
- **Static analysis (`make static-check` = `make lint` + `make mermaid-lint` + `make diagrams-check`)**: `make lint` hadolints all five Dockerfiles via a pinned `hadolint/hadolint` image (`.hadolint.yaml` sets `failure-threshold: error`, so `DL3008` apt-pin advisories don't fail); `make mermaid-lint` validates the README's remaining Mermaid diagram (the LDAP tree) via a pinned `minlag/mermaid-cli` image; `make diagrams-check` re-renders the C4 PlantUML source and fails on drift. `make static-check` is the composite gate run by the CI `static-check` job and by `make ci`. `make ci-run` runs the act-runnable jobs (`static-check` + `test`) locally. `docs/diagrams/**/*.puml` is in the CI `changes` `code` filter so a diagram-source edit runs the gate.
- **Tooling pins**: `hadolint`, `trivy`, `mermaid-cli` and `plantuml/plantuml` are consumed via pinned `docker run` images (Renovate-tracked `# renovate:` Makefile pins — the docker-run exception to the mise policy, since this repo has no language toolchain). **`plantuml/plantuml` is Renovate-tracked but automerge-DISABLED** (renovate.json packageRule): its bump forces a diagram re-render the bot cannot perform, so a human runs `make diagrams` + commits the new PNG on the bump PR (the shepherded-bump option — a standing RED automerge PR is avoided). `act` is the one mise-managed tool — pinned in **`.mise.toml`** (`aqua:nektos/act`, tracked by Renovate's native mise manager) and used only by the local `make ci-run`; CI never runs `act`, so no job needs `jdx/mise-action`.
