...
- `jaasAuthenticationPlugin configuration="LDAPLogin"` — authentication. The `LDAPLogin` realm is defined in `5.19.6/conf/login.config` (`org.apache.activemq.jaas.LDAPLoginModule`). An alternate `ldaptive`-based realm also lives in that file but is not the active one.
...
- **Jetty (9.4.58, matching ActiveMQ 5.19.6's bundled Jetty 9.4.x) and ldaptive (2.5.0) jars** are curl'd from Maven Central at image-build time into the broker's `lib/` — they are not part of the stock ActiveMQ distribution and are needed for the JAAS/LDAP web-console auth. `JETTY_VER` must track the broker's bundled Jetty (`activemq-parent` pom `jetty9-version`).
- The base image is `eclipse-temurin:11-jre` — ActiveMQ 5.19.6 requires Java 11 (`source`/`target` = 11 in its parent pom; the Java-8 javadoc URL in that pom is a stale link, not the compile target).
...
_Last updated 2026-05-31. ActiveMQ is intentionally **not** Renovate-tracked — bumping it requires updating `SHA512_VAL` in `5.19.6/Dockerfile` for the new tarball AND re-aligning `JETTY_VER` to the broker's bundled Jetty (`activemq-parent` pom `jetty9-version`). Jetty/ldaptive (Maven), the Docker images, and Actions ARE Renovate-managed. Landed 2026-05-31 (see git history): ActiveMQ 5.16.1→5.19.6 (Java 8→11, clears CVE-2023-46604 OpenWire RCE), Jetty→9.4.58, ldaptive→2.5.0 (Renovate-bumped from 1.3.3), phpLDAPadmin→leenooks v2, LAM→GHCR 9.5._
...
- [ ] Reconsider the **overlay-jar pattern** — `jetty-{jaas,security,util}` + ldaptive are curled into `lib/` at `JETTY_VER`/`LDAPTIVE_VER`, coupling them to the broker's internals. ActiveMQ 5.19.6 already bundles Jetty 9.4.58; check whether any of the three Jetty overlays are now redundant and overlay only what the distribution lacks (`jetty-jaas`).
- [ ] ldaptive is now **2.5.0** (Renovate-bumped 2026-05-31). The unused `ldaptive-activemq-ldap` realm in `login.config` / `login-alt.config` still references the 1.x JAAS API (`org.ldaptive.jaas.LdapLoginModule`) — harmless today (that realm is never activated; the live path is ActiveMQ's own `LDAPLoginModule`, e2e green), but rewrite it to the 2.x API or delete the dead realm.
