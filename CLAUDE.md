...
- `.github/workflows/docker-image.yml` — CI (`name: CI`): matrix-builds both Dockerfiles + **blocking** Trivy scan (`exit-code: '1'`, waivers in `.trivyignore`), `test` (bats + mermaid-lint), `e2e` (compose authZ contract), and `e2e-samba` (Samba AD DC serves LDAP) jobs.
- `.trivyignore` — documented CVE waivers for the broker image: the EOL ActiveMQ-bundled Spring 5.3 / Jetty 9.4 jars whose only fix is the ActiveMQ 6.x migration; each entry carries a reachability note + source.
...
- [x] **CI Trivy scan flipped to BLOCKING (`exit-code: '1'`, 2026-05-31).** Re-baselined on the Java-25 base (14 fixable HIGH/CRITICAL): removed the genuinely-removable findings outright — the dormant `lib/camel` jars (no `<camelContext>` in `activemq.xml`) and Canonical's `pebble` Go binary baked into both bases (8 Go-stdlib CVEs per image). That left **5** broker CVEs in EOL ActiveMQ-bundled Spring 5.3 / Jetty 9.4 jars whose only fix is ActiveMQ 6.x; each is unreachable in this broker and is documented + waived in [`.trivyignore`](.trivyignore) with per-CVE reachability notes + sources. The samba image scans clean. The gate now hard-fails on any **new fixable** CVE in either image.
- [ ] **ActiveMQ 6.x migration (epic).** The 5.19.x line is permanently pinned to EOL Spring 5.3 / Jetty 9.4 / Camel 2.x (`javax.*`); the latest 5.x (5.19.7) bundles byte-identical versions, so no 5.x bump clears the `.trivyignore`-waived CVEs. The only path onto patched dependency lines is **ActiveMQ 6.x** (Jakarta namespace, Spring 6.1.x, Jetty 11.x, Java 17+) — a migration (`jakarta.jms.*` rework, Jetty-11 console `jetty.xml` rework, JAAS re-verify), not a drop-in upgrade. Tracked as a separate epic.
