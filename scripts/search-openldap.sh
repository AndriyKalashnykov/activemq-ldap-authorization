...
docker exec openldap ldapsearch -x -H ldap://localhost:389 -b ou=Group,ou=ActiveMQ,dc=activemq,dc=apache,dc=org -s sub "(member=uid=user)" -D "cn=admin,dc=activemq,dc=apache,dc=org" -w admin
